...
Page.Game = new DrawAreaObj(0,0,10,20,function(){
  ...
  if (GM.Pc.Cur !== 0 && GM.IsAlive){
    ...
    for(const element of GM.Pc.Cur.UO.arr){
      drawL = (GM.Pc.Cur.x + element.x) * Page.unitSize + 1;
      drawT = (GM.Pc.Cur.y + element.y) * Page.unitSize + 1;
...
Page.UpcomingA = new DrawAreaObj(10.5,2.6,2.5,2.5,function(){
  ...
  if (pcA !== 0){
    ...
    for(const element of pcA.UO.arr){
      let curX = element.x,
          curY = element.y;
    ...
    for(const element of pcA.UO.arr){
      let drawL = Math.floor(offsetL - avgL + element.x * uDrawSize),
          drawT = Math.floor(offsetT - avgT + element.y * uDrawSize); 
...
Page.UpcomingB = new DrawAreaObj(10.5,5.2,2.5,2.5,function(){
  ...
  if (pcB !== 0){
    ...
    // calculate average positions of units in order to center
    for(const element of pcB.UO.arr){
      let curX = element.x,
          curY = element.y;
...
Page.UpcomingC = new DrawAreaObj(10.5,7.8,2.5,2.5,function(){
  ...
  if (pcC !== 0){
    ...
    // calculate average positions of units in order to center
    for(const element of pcC.UO.arr){
      let curX = element.x,
          curY = element.y;
...
let GM = {
  ...
  CheckUnits: function(checkRowsRaw){
    ...
    for(const element of checkRows){
      let hasGap = false,
          checkIndex = element
      
      for(const element of GM.StaticUnits){
        if (element[checkIndex] === 0){
          hasGap = true;
          break;
        }
      }
...
GM.Pc = {
  ...
  Freeze: function(){
    ...
    if (GM.IsAlive){
      ...
      for(const element of this.Cur.UO.arr){
        let staticX = this.Cur.x + element.x,
            staticY = this.Cur.y + element.y;
  ...
  CheckCollisions: function(doRot, offsetX, offsetY){
    ...
    for(const element of unitArr){
      let testX = this.Cur.x + element.x + offsetX,
          testY = this.Cur.y + element.y + offsetY,
...
function DrawText(text, color, weight, alignment, size, left, top){  
  Page.ctx.font = weight + ' ' + size + 'px "SFMono-Regular", sans-serif';
